...
class App extends Component {
  ...
  markComplete = id => {
    this.setState({
      todos: this.state.todos.map(todo => {
        if (todo.id === id) {
          // match the id & cross a line through when checkbox is clicked
  ...
  delTodo = id => {
    axios
      .delete(`https://jsonplaceholder.typicode.com/todos/${id}`) //we use back ticks because we also need id to delete
      .then(res =>
        this.setState({
          todos: [...this.state.todos.filter(todo => todo.id !== id)]
        })
      ); //we use filter array method to loop through & return what we define
  };
